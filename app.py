...
def get_compatibility_analysis(user_mbti, user_astro, groupmates):
    overall_score, individual_scores = calculate_compatibility(
        user_mbti, user_astro, groupmates
    )
    details = []
    ...
    for index, (groupmate_mbti, groupmate_astro) in enumerate(groupmates):
        ...
        try:
            response = openai.Completion.create(
                ...
                prompt=prompt,
                max_tokens=500,  # to make sure lines don't cut-off
...
def submit():
    if request.method == "POST":
        ...
        overall_score, details = get_compatibility_analysis(
            user_mbti, user_astro, groupmates
        )
        print(f"Overall Score: {overall_score}")
        print(f"Details: {details}")

        results = {"overall_score": overall_score, "details": details}
